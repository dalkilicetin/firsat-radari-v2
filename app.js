...
function evaluate(sym){
  ...
  const killCo = [], killWorld = [];
  if(th && th.risk) killWorld.push(th.risk);
  if(pats.includes("PAHALI")) killCo.push("çarpan daralması");
  const eNext = (earnCal.list||[]).find(z=>z.s===sym);
  if(eNext) killCo.push("kötü bilanço ("+eNext.d+")");
...
function thesis(sym, ev){
  const th = ev.th;
  const parts = [];
  if(th){
    if(th.drivers && th.drivers[0]) parts.push(th.drivers[0]+".");
    const note = (TICKER_THEME[sym] && TICKER_THEME[sym].note) ? " ("+TICKER_THEME[sym].note+")" : "";
    parts.push(((prof[sym]||{}).name||sym)+", "+th.theme.toLowerCase()+" zincirinin "+th.branch.toLowerCase()+" katmanında"+note+".");
  }
  if(ev.kalite>=2) parts.push("Şirket kalite tarafında güçlü: "+ev.kalTxt.slice(0,2).join(", ")+".");
  else if(ev.kalite===1) parts.push("Kalite tarafı kısmi: "+ev.kalTxt[0]+".");
  if(ev.kat>=1) parts.push("Zamanlama sinyali: "+ev.katTxt.slice(0,2).join(", ")+".");
  const risks = [...(ev.killCo||[]).slice(0,1), ...(ev.killWorld||[]).slice(0,1)];
  if(risks.length) parts.push("En büyük risk: "+risks.join(" ve ")+".");
  parts.push(ev.verdict+".");
  return parts.join(" ");
}
